...
def epoch(loader, model, opt=None, ep=-1):
    ...
    abs_err, type_err, type_cnt = 0.0, torch.zeros(NUM_BOND_ORIG_TYPES), torch.zeros(NUM_BOND_ORIG_TYPES,
                                                                                     dtype=torch.long)
    ...
    with torch.enable_grad() if opt else torch.no_grad():
        ...
        for x_idx, x_atom, x_atom_pos, x_bond, x_bond_dist, x_triplet, x_triplet_angle, x_quad, x_quad_angle, y in loader:
            ...
            if args.batch_chunk > 1:
                ...
                y_pred, _ = para_model(x_atom, x_atom_pos, x_bond, x_bond_dist, x_triplet, x_triplet_angle, x_quad,
                                       x_quad_angle)
            ...
            if opt:
                ...
                if args.batch_chunk == 1:
                    ...
                    if args.champs_loss:
                        ...
                        raw_loss = torch.log(
                            (b_type_err[nonzero_indices] / b_type_cnt[nonzero_indices].float()) + 1e-9).mean()
                    if APEX_AVAILABLE:
                ...
                if batch_id % log_interval == 0:
                    ...
                    logging(
                        f"Epoch {ep:2d} | Step {train_step} | lr {opt.param_groups[0]['lr']:.7f} | Error {avg_loss:.5f}")

                    total_loss = 0

    torch.cuda.empty_cache()
    return abs_err / type_cnt.sum(), torch.log(type_err / type_cnt.float()).mean(), torch.log(
        type_err / type_cnt.float())
