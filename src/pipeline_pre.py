...
def add_embedding(atoms, bonds, triplets, quadruplets, embeddings=None):
    ...
    for t in range(1):
        quadruplets["type_index_" + str(t)] = quadruplets["type_" + str(t)].apply(
            lambda x: embeddings[('quadruplet', t)][x])
    return embeddings
...
def create_dataset(atoms, bonds, triplets, quads, labeled=True, max_count=10 ** 10):
    ...
    for k, i in tqdm(index.items()):
        ...
        if mf > 0:
            ...
            for t in range(3):
                x_bond[i, mr:mr + top_count, t] = torch.tensor(mol_fake_bonds["type_index_" + str(t)].values)[
                                                      argsort_fdists][:top_count]
            x_bond[i, mr:mr + top_count, 3] = torch.tensor(mol_fake_bonds["atom_index_0"].values)[argsort_fdists][
                                              :top_count]
            x_bond[i, mr:mr + top_count, 4] = torch.tensor(mol_fake_bonds["atom_index_1"].values)[argsort_fdists][
                                              :top_count]
        ...
        lookup.update(
            dict(zip(mol_real_bonds["atom_index_1"].apply(str) + "_" + mol_real_bonds["atom_index_0"].apply(str),
                     range(mol_real_bonds.shape[0]))))
        ...
        y_bond_scalar_coupling[i, :mr, 3] = torch.tensor(
            mol_real_bonds["predict"].values).float()  # binary tensor (1s to be predicted)
...
def auto_preproc_stage2():
    ...
    quadruplets.sort_values(['molecule_name', 'atom_index_0', 'atom_index_1', 'atom_index_2', 'atom_index_3'],
                            inplace=True)
